...
let myTable = {
    ...
    function printTitleRow(){
        const tableWidth = myTable.columnsSetting.reduce((accu, curr)=>accu + curr.width, 0);
        let spaceCount = Math.ceil((tableWidth - myTable.title.length) / 2);
    ...
    function sortRows(){
        const sortTitle = myTable.sortByTitle;
        const column = myTable.columnsSetting.find(a => a.title === sortTitle);
    ...
    function sortRowsByDefault(key){
        const emptyRows = myTable.rows.filter(a => a[key] === "");
        myTable.rows = myTable.rows.filter(a=>a[key] !== "").sort((a,b)=>a[key].toLowerCase().localeCompare(b[key].toLowerCase()));
        myTable.rows = myTable.rows.concat(emptyRows);
        let id = 1;
    ...
    function sortRowsByDate(key){
        ...
        const emptyRows = myTable.rows.filter(a => a[key] === "");
        let notEmptyRows = myTable.rows.filter(a=>a[key] !== "");
        ...
        myTable.urgentDate = myTable.rows.filter(a =>{
...
function sortReadline(){
    rl.question("Enter title : ", (input)=>{
        ...
        if(column === undefined){
            ...
            myTable.sortByTitle = column.title;
            showMain();
        }
    });
}
...
function configRemoveCol(){
    rl.question("Enter column title : ", (input)=>{
        const removeKey = input.trim().toLowerCase();
        const colIndex = myTable.columnsSetting.findIndex(a=>a.key === removeKey);
        if(colIndex === NO_FIND){
            console.log("This column doens't exist in table.");
            commandList(); 
        }else if(removeKey === "id"){
            console.log("This column is not allowed to be removed.");
            commandList(); 
        }
        rl.question(`Do you really want to remove ${COLOR_RED}${removeKey}${COLOR_RESET} column?\n${COLOR_RED}All content will be lost. y/n ${COLOR_RESET}: `,(answer)=>{
            if (answer.trim().toLowerCase() === 'y') {
...
function convertColumnTypeDate(key){
    const contentArr = myTable.rows.map(row => row[key]).filter(row => row !== "");
